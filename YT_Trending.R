...
```{r}
cat <- as.numeric(clean$date_diff)
ggplot(clean, aes(x=cat)) + 
  geom_histogram(binwidth= 1, color='white', aes(fill=..count..)) + 
  labs(title = 'How Many Days Until a Video Starts Trending?', x='Days', y='Frequency')+ 
  xlim(0,10) + 
  scale_fill_gradient('count', low='coral', high='darkgoldenrod1')
```
...
cat24 <- as.numeric(data24$date_diff)
ggplot(data24, aes(x=cat24)) + 
  geom_histogram(binwidth= 1, color='white', aes(fill=..count..)) + 
  labs(title = 'Entertainment: How Many Days Until a Video Starts Trending?', x='Days', y='Frequency') + 
  xlim(0,10) + 
  scale_fill_gradient('count', low='deepskyblue', high='cornflowerblue')
summary(cat24)
boxplot(cat24)
```
...
cat10 <- as.numeric(data10$date_diff)
ggplot(data10, aes(x=cat10)) + 
  geom_histogram(binwidth= 1, color='white', aes(fill=..count..)) + 
  labs(title = 'Music: How Many Days Until a Video Starts Trending?', x='Days', y='Frequency') + 
  xlim(0,10) + 
  scale_fill_gradient('count', low='deepskyblue', high='cornflowerblue')
summary(cat10)
boxplot(cat10)
...
cat26 <- as.numeric(data26$date_diff)
ggplot(data26, aes(x=cat26)) + 
  geom_histogram(binwidth= 1, color='white', aes(fill=..count..)) + 
  labs(title = 'How to and Style: How Many Days Until a Video Starts Trending?', x='Days', y='Frequency') + 
  xlim(0,10) + 
  scale_fill_gradient('count', low='deepskyblue', high='cornflowerblue')
summary(cat26)
boxplot(cat26)
